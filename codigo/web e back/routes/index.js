...
router.get('/additemWeb/edit', (req,res)=>{
    var id = localStorage.getItem('id')
    Auction.findById(id, function(err, auction){
        localStorage.setItem('nameAuction',auction.name)
        localStorage.setItem('description',auction.description)
        localStorage.setItem('date',auction.endDate)
        res.render('additem',{edit:true})
    })
})
        ...
        if(response.success == true){
            Auction.find().lean().then((auctions) => {
                ...
                res.render('dashboard', {auctions: auctions, email: req.body.email})
                 ...
                 res.render('login', {error: true, msg:'Erro no login'})
            })
        }
        else{
            res.render('login',{error: true, msg:'Erro no login, senha e/ou email errados'})
        }
    })
})
