...
router.get('/user/:id', (req, res) => {
    User.findById(req.params.id, function (err, user) {
        if (err) res.redirect('dashboard')
            ...
            var numT = user.counttransaction
...
router.post('/transaction/:id/:involvedEmail/:negotiatorEmail', (req, res) => {
    if (req.body.note > 5 || req.body.note < 1) {
        var email = localStorage.getItem('email')
        Transaction.find({ "involvedEmail": email }).lean().then((trans) => {
            res.render('transaction', { transactions: trans, error: true, msg: 'A nota deve ser entre 1 e 5' })
        })
    }
    else {
        User.findOne({ "email": req.params.negotiatorEmail }, function (err, user) {
            var media = user.reputation
            var count = user.counttransaction + 1
            if (media == -1) {
                media = req.body.note
            } else {
                media = parseInt(req.body.note) + parseInt(user.reputation)
                media = media / 2
            }
