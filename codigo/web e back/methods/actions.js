            ...
            Item.findOne({
                ...
                if (!item) {
                    ...
                    if (req.body.bid <= item.price)
                        ...
                        historic.push(req.body.personName + "-" + req.body.bid)
                        if (item.historic != undefined) {
                            dataActions.sendnotificationemail(req.body.personName, item.hightestbidderEmail, item.name)
                        }
                        Item.findOneAndUpdate({
                        ...
                        },
                            {
                                $set: {
                                    "historic": historic,
                                    "hightestbidder": req.body.personName,
                                    "name": item.name,
                                    "price": req.body.bid,
                ...
                Item.findOneAndUpdate({
                ...
                },
                    {
                        ...
                        if (err) {
                            ...
                            res.json({ success: true, msg: 'Mensagem postada com sucesso', forum: forum })
                        }
                    })
            }
        }
        )
    }, returndata: function (req, res) {
        Item.findOne({
            name: req.body.itemName,
            linkedAuction: req.body.linkedAuction
        }, function (err, item) {
            if(err) throw err
            else res.json({forum:item.forum, historico: item.historic})
        })
    }
}
