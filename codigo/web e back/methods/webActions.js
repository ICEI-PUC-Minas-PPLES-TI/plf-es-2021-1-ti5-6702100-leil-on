    ...
    search: function (req, res, linkedAuction) {
        if ((!req.body.type) || (!req.body.obj)) {
            ...
            switch (req.body.type) {
                case "Leilão":
                Auction.find({"name":req.body.obj}).lean().then((auctions) => {
                    res.render('dashboard', {auctions: auctions})
                }).catch((err)=>{
                    res.redirect('/dashboard')
                })
                    break;
                case "Item":  Item.find({"name":req.body.obj}).lean().then((itens) => {
                    res.render('dashboard', {itens: itens})
                }).catch((err)=>{
                    res.redirect('/dashboard')
                })
                    break;
                case "Usuário":  User.find({"name":req.body.obj}).lean().then((users) => {
